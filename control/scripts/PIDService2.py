...
class PIDService2:
	...
	def handle_pid_compute(self,req):
		if req.pen_pos_up:
			...
			rospy.loginfo(f'compute_2: pen up gains...{[self.kp_up,self.kd_up,self.ki_up]}')
		else:
			...
			rospy.loginfo(f'compute_2: pen down gains...{[self.kp_down,self.kd_down,self.ki_down]}')
